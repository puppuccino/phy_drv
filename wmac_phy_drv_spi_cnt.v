...
module wmac_phy_drv_spi_cnt (
    ...
    always @(posedge clk_spi or negedge nrst) begin
        ...
        else if (trans_done) o_cs_n <= 1'b1;
        else if (req && stat == 6'd0) o_cs_n <= 1'b0;
        // explicit hold: else o_cs_n <= o_cs_n;
    end

    always @(posedge clk_spi_n or negedge nrst) begin
        if (~nrst) o_mosi <= 1'b0;
        else if (~o_cs_n) o_mosi <= mosi_nxt;
        // explicit hold: else o_mosi <= o_mosi;
    end

    always @(posedge clk_spi or negedge nrst) begin
        if (~nrst) rdat <= 16'b0;
        else if (~o_cs_n && com[1:0] == 2'b11) rdat <= rdat_nxt;
        // explicit hold: else rdat <= rdat;
    end

    always @(posedge clk_spi or negedge nrst) begin
        if (~nrst) stat <= 6'b0;
        else if (~req) stat <= 6'b0;
        else begin
            stat <= (done) ? 6'b0 : stat + 1'b1;
        end
    end
